 ...
 * to get a series of error messages related to the failure. This function must be called before any resources
 * are cleaned up or freed.
 *
 * @param object Pointer to the object that returned the failure status. Use `NULL` for init failures or functions that
 *               do not take an object pointer.
 * @param[out] message_stack Array of messages relating to the failure. Messages are NULL terminated strings.
 *                           The array and messages are freed when the associated object is deleted.
 * @param[out] message_stack_depth The number of messages in the `message_stack` array.
 */
PV_API void pv_get_error_stack(
        const void *object,
